...
M2Lib::EError M2Lib::M2I::Load(wchar_t const* FileName, M2Lib::M2* pM2, bool IgnoreBones, bool IgnoreAttachments, bool IgnoreCameras, bool IgnoreOriginalMeshIndexes)
{
	...
	for (uint32_t i = 0; i < InSubsetCount; i++)
	{
		...
		// NOTE[VERTICES]: Bigger max vertices value
		if (VertexList.size() + InVertexCount > 0x7FFFFF)
		...
		for (uint32_t j = 0; j < InVertexCount; ++j)
		{
			...
			uint32_t VertexIndex = (uint32_t)VertexList.size();
			VertexList.push_back(InVertex);
			pNewSubMesh->Indices.push_back(VertexIndex);

			// NOTE[DEBUG]
			if (VertexList.size() > UINT16_MAX) {
				sLogger.LogInfo(L"[M2I::Load] WARNING VertexList.size() is bigger than uint16 max: %i", VertexList.size());
			}
		...
		for (uint32_t j = 0; j < InTriangleCount; ++j)
		{
			...
			for (uint32_t k = 0; k < VERTEX_PER_TRIANGLE; ++k) {
				uint32_t valueReadAndProcessed = DataBinary.Read<uint16_t>() + VertexStart;
				NewTriangle.Vertices[k] = (uint32_t)valueReadAndProcessed;

				// NOTE[DEBUG]
				if (valueReadAndProcessed > UINT16_MAX) {
					sLogger.LogInfo(L"[M2I::Load] WARNING NewTriangle.Vertices[k] (valueReadAndProcessed) is bigger than uint16 max: %i", valueReadAndProcessed);
				}
			}
