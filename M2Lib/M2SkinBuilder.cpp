...
bool M2Lib::M2SkinBuilder::Build(M2Skin* pResult, uint32_t BoneLoD, M2I* pM2I, CVertex* pGlobalVertexList, uint32_t BoneStart)
{
	...
	for (uint32_t i = 0; i < m_SubMeshList.size(); ++i)
	{
		for (uint32_t j = 0; j < m_SubMeshList[i]->SubsetPartitions.size(); ++j)
		{
			...
			std::map< uint32_t, uint32_t > GlobalToSkinIndexMap;
			...
			for (uint32_t k = 0; k < pSubsetPartition->Triangles.size(); ++k)
			{
				...
				{
					uint32_t VertexToMap = pSubsetPartition->Triangles[k]->Vertices[iVert];	// this is the global vertex index
					uint32_t VertexMapped = 0;
					if (GlobalToSkinIndexMap.find(VertexToMap) != GlobalToSkinIndexMap.end())
						VertexMapped = GlobalToSkinIndexMap[VertexToMap];
					else
					{
						VertexMapped = (uint32_t)m_Vertices.size();
					...
					// NOTE[DEBUG]
					if (VertexToMap > UINT16_MAX) {
						sLogger.LogInfo(L"[M2SkinBuilder::Build] WARNING VertexToMap is bigger than uint16 max: %i", VertexToMap);
					}
					if (VertexMapped > UINT16_MAX) {
						sLogger.LogInfo(L"[M2SkinBuilder::Build] WARNING VertexMapped is bigger than uint16 max: %i", VertexMapped);
					}
	...
	pResult->Elements[M2SkinElement::EElement_VertexLookup].SetDataSize(m_Vertices.size(), m_Vertices.size() * sizeof(uint32_t), false);
	uint32_t* Indices = pResult->Elements[M2SkinElement::EElement_VertexLookup].as<uint32_t>();
	memcpy(Indices, m_Vertices.data(), m_Vertices.size() * sizeof(uint32_t));

	pResult->Elements[M2SkinElement::EElement_TriangleIndex].SetDataSize(m_Indices.size(), m_Indices.size() * sizeof(uint32_t), false);
	uint32_t* Triangles = pResult->Elements[M2SkinElement::EElement_TriangleIndex].as<uint32_t>();
	memcpy(Triangles, m_Indices.data(), m_Indices.size() * sizeof(uint32_t)); // triangles
	...
	for (uint32_t i = 0; i < m_SubMeshList.size(); i++)
	{
		for (uint32_t j = 0; j < m_SubMeshList[i]->SubsetPartitions.size(); ++j)
		{
			...
			// NOTE[DEBUG]
			// FINDING[VERTICES]: Yes pSubsetPartitionIn->VertexStart is greather than uint16 max...
			if (pSubsetPartitionIn->VertexStart > UINT16_MAX) {
				sLogger.LogInfo(L"[M2SkinBuilder::Build] WARNING: pSubsetPartitionIn->VertexStart is greather than uint16 max: %i", pSubsetPartitionIn->VertexStart);
			}
