...
M2Lib::EError M2Lib::M2Skin::Load(wchar_t const* FileName)
{
	...
	if ((Header.ID[0] != 'S' || Header.ID[1] != 'K' || Header.ID[2] != 'I' || Header.ID[3] != 'N') &&
		(Header.ID[0] != 'S' || Header.ID[1] != 'K' || Header.ID[2] != 'I' || Header.ID[3] != 'X'))
...
void M2Lib::M2Skin::BuildVertexBoneIndices()
{
	...
	uint32_t* VertexLookupList = Elements[EElement_VertexLookup].as<uint32_t>();
...
void M2Lib::M2Skin::CopyMaterials(M2Skin* pOther)
{
	...
	// EDITED[HMM]: However, check for the size of the flags first (avoids model crashes)
	if (Elements[EElement_Flags].Count > 0) {
